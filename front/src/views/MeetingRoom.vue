    ...
    <div class="meetingroom" :class="{'changeWidth': ismargin}">
      <div class="sidebar" :class="{'sidewidth':isOpen}">
        <div class="btn-zip">
          ...
          <label for="chat-btn" class="btn-item chat-btn" @click="setchat">채팅</label>
          <input type="radio" id="parti-btn" name="button">
          <label for="parti-btn" class="btn-item parti-btn" @click="setparticipant">참가자</label>
          <input type="radio" id="record-btn" name="button">
          <label for="record-btn" class="btn-item record-btn" @click="setrecord">기록</label>
        </div>
    ...
    <div class='meeting-bottom'>
      <div class="bottom-btn-box">
        <div class="circle-btn">
          <i class="fas fa-camera fa-2x" @click="colorchangeCamera" :style="{'color':camC, 'background':camB}"></i>
          <p v-if="camera" class="arrow_box">카메라 on</p>
          <p v-if="!camera" class="arrow_box">카메라 off</p>
        </div>
        <div class="onoff-mike circle-btn">
          <i class="fas fa-microphone fa-2x" @click="colorchangemike" :style="{'color':mikeC, 'background':mikeB}"></i>
          <p v-if="mike" class="arrow_box">마이크 on</p>
          <p v-if="!mike" class="arrow_box">마이크 off</p>
        </div>
        <div class="onoff-caption circle-btn">
          <i class="fas fa-closed-captioning fa-2x" @click="colorchangeCaption" :style="{'color':capC, 'background':capB}"></i>
          <p v-if="caption" class="arrow_box">자막 on</p>
          <p v-if="!caption" class="arrow_box">자막 off</p>
        </div>
        <div class="onoff-sign circle-btn">
          <i @click="signVideoOpen" class="fas fa-sign-language fa-2x" :style="{'color':viC, 'background':viB}"></i>
          <p v-if="signVideo" class="arrow_box">수화영상 on</p>
          <p v-if="!signVideo" class="arrow_box">수화영상 off</p>
        </div>
        <div class="sidebar-btn circle-btn">
          <i class="far fa-eye fa-2x" @click="colorchangeSignRecog" :style="{'color':signC, 'background':signB}"></i>
          <p v-if="signRecog" class="arrow_box">수화인식 on</p>
          <p v-if="!signRecog" class="arrow_box">수화인식 off</p>
        </div>
        <div class="sidebar-btn circle-btn">
          <i @click="sideOpen" class="fas fa-ellipsis-v fa-2x" :style="{'color':asC, 'background':asB}"></i>
...
.btn-zip input[type="radio"]:checked + label {
  background-image: linear-gradient(to right top, #7fa8f9, #82a0fe, #8a97ff, #958cff, #a380ff, #a87dff, #ad7bff, #b278ff, #ac80ff, #a887ff, #a38eff, #a094ff);
}
...
.plus-box {
  width: 100%;
  height: 160px;
  background-image: linear-gradient(to right top, #b7b1fa, #b4a6fd, #b39bff, #b38eff, #b581ff, #b47eff, #b27bff, #b178ff, #ab80ff, #a587ff, #a08eff, #9c94ff);
  z-index: 2;
  /* position:absolute; */
  margin-bottom: 80px;
  left: 0;
