...
function serverHandler(request, response) {
  ...
  try {
    ...
    try {
      ...
      if (
        filename.search(/voda/g) === -1 &&
        filename.search(/admin/g) === -1 &&
        stats.isDirectory() &&
        config.homePage === "/voda/index.html"
      ) {
        if (response.redirect) {
          response.redirect("/voda/");
        } else {
    ...
    try {
      if (fs.statSync(filename).isDirectory()) {
        ...
        if (filename.indexOf(resolveURL("/voda/member/")) !== -1) {
          console.log(filename);
          filename = filename.replace(resolveURL("/voda/member/"), "");
          console.log(filename);
          filename += resolveURL("/voda/member/index.html");
          console.log(filename);
        } else if (filename.indexOf(resolveURL("/voda/create/")) !== -1) {
          console.log(filename);
          filename = filename.replace(resolveURL("/voda/create/"), "");
          console.log(filename);
          filename += resolveURL("/voda/create/index.html");
          console.log(filename);
        } else if (filename.indexOf(resolveURL("/voda/join/")) !== -1) {
          console.log(filename);
          filename = filename.replace(resolveURL("/voda/join/"), "");
          console.log(filename);
          filename += resolveURL("/voda/join/index.html");
          console.log(filename);
        ...
        } else if (filename.indexOf(resolveURL("/voda/dashboard/")) !== -1) {
          filename = filename.replace(resolveURL("/voda/dashboard/"), "");
          filename += resolveURL("/voda/dashboard/index.html");
        } else if (
          filename.indexOf(resolveURL("/voda/video-conference/")) !== -1
        ) {
          filename = filename.replace(
            resolveURL("/voda/video-conference/"),
            ""
          );
          filename += resolveURL("/voda/video-conference/index.html");
        } else if (filename.indexOf(resolveURL("/voda")) !== -1) {
          filename = filename.replace(resolveURL("/voda/"), "");
          filename = filename.replace(resolveURL("/voda"), "");
          filename += resolveURL("/voda/index.html");
        } else {
